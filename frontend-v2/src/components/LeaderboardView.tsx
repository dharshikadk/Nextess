...
export const LeaderboardView: React.FC<LeaderboardViewProps> = ({
  ...
  if(!board.opened||board.entries.length===0)return <div className="flex flex-col w-full pb-20"><div className="py-3"><span className="font-mono text-xs uppercase tracking-wider text-violet-400 font-bold">Tournament Hub</span><h1 className={`font-headline-lg text-2xl md:text-3xl font-bold tracking-tight ${isDark?'text-white':'text-slate-900'}`}>Streaks &amp; Leaderboard</h1></div><div className={`rounded-2xl p-10 border shadow-xl text-center ${isDark?'bg-[#12131b] border-violet-500/20':'bg-white border-slate-200'}`}><span className="material-symbols-outlined text-[48px] text-violet-400">leaderboard</span><h2 className={`mt-4 text-xl font-bold ${isDark?'text-white':'text-slate-900'}`}>Leaderboard is not opened</h2><p className="mt-2 text-sm text-slate-400">The 3-Day Master League has no active participants yet. Log in and join when the league opens.</p></div></div>;
  return (
